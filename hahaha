...
<a href="#" onclick="c(event);unmuteVideo(event)" id="btn" style="position:fixed;top:20px;right:20px;z-index:100;font-size:24px;color:white;background:rgba(0,0,0,0.7);padding:10px;border-radius:5px;text-decoration:none;display:none;">кликни</a>
<script>
function unmuteVideo(e) {
    let video = document.getElementById('video');
    video.contentWindow.postMessage({method: 'setVolume', value: 1}, '*');
    video.src = video.src.replace('&muted=1', '') + '?autoplay=1&loop=1';
}
